...
> 打补丁**（`git apply flutter_tools.patch`，见[快速开始](#快速开始)），且 SDK
> 版本需要匹配 `pubspec.yaml` 中声明的支持范围。仅添加依赖、不打补丁不会有任何效果。
...
AOPD 面向 `pubspec.yaml` 中声明的 Flutter/Dart SDK 支持范围。包内使用严格的 Dart SDK
约束，让不支持的 SDK 版本在依赖解析阶段直接失败，而不是等到编译阶段才报错。版本支持变化记录在
`CHANGELOG.md` 中。
...
请尽量把项目放在较短且不包含中文字符的路径下。中文路径或过长路径可能导致
frontend-server snapshot 编译失败。
